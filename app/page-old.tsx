...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedSite) {
      ...
      // Check if there are any active jobs for this site
      checkJobStatus(selectedSite.id);
    }
  }, [selectedSite]);

  const checkJobStatus = async (siteId: string) => {
    try {
  ...
  useEffect(() => {
    if (!selectedSite || !collecting) return;

    const pollJobStatus = async () => {
      try {
        const jobStatus = await api.getJobStatus();
        const siteStatus = jobStatus.sites.find(s => s.siteId === selectedSite.id);

        if (siteStatus) {
          // If site is idle and we're still collecting, the job completed
          if (siteStatus.status === 'idle' && collecting) {
            setCollecting(false);
            setCollectionError(null);
          ...
          else if (siteStatus.status !== 'idle' && !collecting) {
            setCollecting(true);
            setCollectionError(null);
          }
        }
                          ...
                          {collectionError.includes('temporarily') && (
                            <div className="mt-3">
                              <button
                                ...
                                className="text-sm bg-yellow-100 hover:bg-yellow-200 text-yellow-800 px-3 py-1 rounded-md border border-yellow-300 transition-colors"
                              >
                                Try Again
                              </button>
                            </div>
