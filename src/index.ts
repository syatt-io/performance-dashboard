...
async function startServer() {
  ...
  if (monitoringEnabled) {
    ...
    logger.info('⏸️ Performance monitoring disabled (set MONITORING_ENABLED=true to enable)');
  }

  app.listen(PORT, () => {
    logger.info(`Performance Dashboard API running on port ${PORT}`);
    logger.info(`Health check: http://localhost:${PORT}/health`);
    logger.info(`API endpoints available at: http://localhost:${PORT}/api`);
    if (monitoringEnabled) {
      logger.info(`📊 Automatic monitoring: ENABLED (every ${process.env.LIGHTHOUSE_INTERVAL_HOURS || 6} hours)`);
    }
  });
}
